...
plt.figure(1)
plt.errorbar(1+np.arange(n),lattice_train_accuracy.mean(axis=1),lattice_train_accuracy.std(axis=1), label='Lattice CNN',alpha=0.5)
...
plt.ylim((0, 1))
plt.xlim((0, n+1))
plt.savefig('./training' + str(n_experiment) + '.png',dpi=300)
plt.show()


plt.figure(2)
plt.errorbar(1+np.arange(n), lattice_test_accuracy.mean(axis=1), lattice_test_accuracy.std(axis=1), label='Lattice-based CNN',alpha=0.5)
...
plt.ylim((0, 1))
plt.xlim((0, n+1))
plt.savefig('./testing' + str(n_experiment) + '.png',dpi=300)
plt.show()
